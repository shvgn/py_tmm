...
def m2prod(*mxarray):
    '''For passed array of matrices 2x2:
        [m1, m2, m3, m4, ...]
...
def m2prod_test(n):
    ...
    rand = np.random.rand
    mxarr = [np.matrix([[rand(), rand()], [rand(), rand()]])
             for i in range(n)]
    ...
    for i in range(len(mxarr)):
        ...
        print m2prod(*[mxarr[k] for k in
                       range(len(mxarr) - 1, len(mxarr) - i - 2, -1)])
...
class Interval(object):
    ...
    def __repr__(self):
        return "Interval: " + str(self.width) + " width, V: [" + \
            str(self.v_start) + ", " + str(self.v_end) + \
...
class IntervalStack(object):
    ...
    def __process(self):
        ...
        vmin = min([min(interval.v_start, interval.v_end)
        ...
        for i in range(len(self.data)):
            ...
            if i == 0:
                ...
                coordstr = "["  + str(self.dots[i]['coordinate']) + \
                           ", " + str(self.dots[i]['coordinate'] + \
                                      self.data[i].width) + "]"
            vstr = "V: [" + str(self.data[i].v_start) + ", " + \
                            str(self.data[i].v_end) + "]"
...
class MatterExpInterval(Interval):
    ...
    """Rectangle region for calculating quasi-bound states
    in arbitral potential"""
    ...
    def __init__(self, width, height, mass):
        ...
        self.label = ''
        super(MatterExpInterval, self).__init__(width=width, angle=0.0,
                                                v_start=self.height, mass=mass)
...
class MatterStack(IntervalStack):
    ...
    def __init__(self, intervals, showfict=False):
        #, ficticious_width=1e-3, ficticious_mass=0.25):
    ...
    def __process(self):
        ...
        if self.showfict:
            # Fictitious region
            self.data.insert(0,
                             MatterExpInterval(self.data[0].width, 0, minmass))
        else:
        ...
        for i in range(len(self.data) - 1):
            ...
            self.dots.append({
                ...
                'height_ev':
                    erg2eV(self.data[i + 1].height - self.data[i].height),
                'coordinate': coordinate
            })
        for i in range(1, len(self.data) - 1):
            if self.data[i - 1].height > self.data[i].height < \
                    self.data[i + 1].height:
                self.data[i].label = 'well'
            elif self.data[i].height < self.data[i - 1].height or \
                    self.data[i].height < self.data[i + 1].height:
        ...
        for i in range(len(self.data)):
            ...
            if i == 0:
                ...
                coordstr = "["  + str( self.dots[i - 1]['coordinate'] * 1e7) +\
                           ", " + str((self.dots[i - 1]['coordinate'] +
                                       self.data[i].width) * 1e7) + "] nm"
            vstr = "V = " + str(erg2eV(self.data[i].height)) + " eV"
            repstr += coordstr.ljust(cw) + vstr + " label: " + \
                self.data[i].label
            repstr += "\n"
        return repstr

    def wavenum(self, index, energy):
        return np.sqrt(2 * self.data[index].mass * ME *
                       np.complex(energy - self.data[index].height)) / HBAR

    def get_delta(self, index, energy):
        if index == 0:
            # return np.complex(0)
            # by Evropetcev:
            return -self.data[0].width * self.wavenum(0, energy)
        if index == 1:
            return np.complex(0)
        return self.wavenum(index, energy) * \
            np.sum([interval.width for interval in self.data[1:index]])

    def get_transfer_matrix_wo_mass(self, index, energy):
        '''Generate transfer matrix for passed layer index and energy.
        Effective mass difference is not taken into account.'''
        wnsum = self.wavenum(index, energy) + self.wavenum(index + 1, energy)
        r = (self.wavenum(index, energy) - self.wavenum(index + 1, energy)) \
            / wnsum
        t = 2 * self.wavenum(index, energy) / wnsum
        d = self.wavenum(index, energy) * self.data[index].width
        return 1 / t * np.matrix([[np.exp(-1j * d), r * np.exp(-1j * d)],
                                  [r * np.exp(1j * d), np.exp(1j * d)]])

    def get_transfer_matrix_w_mass(self, index, energy):
        '''Generate transfer matrix for passed layer index and energy.'''
        if np.abs(energy - self.data[index].height) <= FAKE_ZERO:
            km_ratio = np.sqrt(self.data[index].mass /
        ...
        kcp = 1 + km_ratio
        kcm = 1 - km_ratio
        kd = 1j * self.wavenum(index, energy) * self.data[index].width
        return 0.5 * np.matrix([[kcp * np.exp(-kd), kcm * np.exp(-kd)],
                                [kcm * np.exp( kd), kcp * np.exp( kd)]])

    def get_transfer_matrix(self, index, energy):
        '''Common method to obtain a transfer matrix.'''
    ...
    def get_interval_object(self, index, energy):
        ...
        uvec = m2prod(*[self.get_transfer_matrix(i, energy) for i in
                        range(index, len(self.data) - 1)]) \
                        * np.matrix([[1], [0]])
        # print uvec
        if self.showfict:
            obj['up'] = uvec[0, 0]
        ...
        obj['wavenum'] = self.wavenum(index, energy)
        return obj

    def get_first_interval_object(self, energy):
        return self.get_interval_object(0, energy)

    def get_energy_range(self, energy_min=None, energy_max=None,
                         stepsnum=CALC_STEPS_NUM):
        if energy_min is None:
            energy_min = 0
        if energy_max is None:
            # Taking second highest barrier
            energy_max = sorted([i.height for i in self.data])[-2]
    ...
    def get_interval_eigen_energy(self, index):
        ...
        lorenz_range = np.array([self.get_lorenzian_ratio(index, e)
                                 for e in energy_range])
        lorenz_deriv = np.array([self.get_lorenzian_ratio_derivative(index, e)
                                 for e in energy_range])
        for i in range(1, len(energy_range) - 1):
            if np.nan in (lorenz_deriv[i - 1], lorenz_deriv[i],
                          lorenz_deriv[i + 1]):
                continue
            elif lorenz_deriv[i - 1] * lorenz_deriv[i + 1] < 0 and \
                lorenz_range[i - 1] < lorenz_range[i] > lorenz_range[i + 1]:
                if VERBOSE:
                    print "***  ***  Peak detected near", \
                        erg2eV(energy_range[i]), "eV!"
                    print "* Derivative before the point:", lorenz_deriv[i - 1]
                    print "* Derivative  after the point:", lorenz_deriv[i + 1]
                eigen_energy.append(self.get_peak_energy(index,
                                    energy_range[i - 1], energy_range[i + 1]))
        return eigen_energy

    def get_eigen_energy(self):
        '''Returns all energy eigen values (in ergs) from all wells'''
        if self.eigen_energy.any(): 
            return self.eigen_energy
        eigen_energy = np.array([]) 
        for index in range(len(self.data)):
            # Stair layers bring bullshit, hence they are taken into account
            if self.data[index].label not in ('well'):
                continue
            for energy in self.get_interval_eigen_energy(index):
                # Note magic number, it's placed here to avoid duplicates
                if np.round(erg2eV(energy), 7) \
                        not in np.round(erg2eV(eigen_energy), 7):
                    eigen_energy = np.append(eigen_energy, energy) 
        self.eigen_energy = np.sort(eigen_energy) 
        return self.eigen_energy

    def get_peak_energy_by_bisections(self, index,
    ...
    def get_peak_energy_by_newtonraphson(self, index, energy_min, energy_max,
                                         tol=CALC_TOLERANCE):
        ...
        while np.abs(f) > tol:
            ...
            df = func(index, energy_mid)
            if VERBOSE:
                print "Mid energy =", rg2eV(energy_mid), \
                    "\tFunc =", f, "\tdFunc =", df
        if VERBOSE:
            print " :::::: Final mid energy:", erg2eV(energy_mid)
        return energy_mid

    def get_peak_energy(self, index,
                        energy_min, energy_max,
                        tol=CALC_TOLERANCE):
        return self.get_peak_energy_by_bisections(
            index, energy_min, energy_max, tol=tol)
        # return self.get_peak_energy_by_newtonraphson(index, energy_min,
                                                    # energy_max, tol=tol)

    def plot_wells_lorenzians(self, energy_min=None, energy_max=None,
                              stepsnum=CALC_STEPS_NUM):
        energy_range = self.get_energy_range(energy_min, energy_max, stepsnum)
        spectra = []
        for i in range(len(self.data)):
            if self.data[i].label in ('well', 'stair'):
                spectra.append(np.array([self.get_lorenzian_ratio(i, e)
                                         for e in energy_range]))
            else:
                spectra.append(None)
        ...
        pl.xlim(xmin=erg2eV(energy_range[0] * 0.9),
                xmax=erg2eV(energy_range[-1] * 1.1))
        pl.grid()
        ...
        for i in range(len(self.data)):
            ...
            axlin.plot(erg2eV(energy_range), spectra[i],
                       label=self.data[i].label + ' \#' + str(i) + " %.1f nm"
                       % (self.data[i].width * toNm))
            axlogy.semilogy(erg2eV(energy_range), spectra[i],
                            label=self.data[i].label + ' \#' + \
                            str(i) + " %.1f nm" % (self.data[i].width * toNm))
        axlin.legend(loc=2)
        axlogy.legend(loc=2)
        pl.show()
    ...
    def get_wavefunction(self, energy, stepsnum=CALC_STEPS_NUM):
        if not energy in self.eigen_energy:
            print "WARNING! Wavefunction of unexisting energy eigen value!"
        # An interval obj = {'energy':  , 'energy_ev': ,
        #                    'up':      , 'un': ,
        #                    'delta':   , 'wavenum'}
        # Width of the ficticious interval, x is negative there
        offset = -self.data[0].width
        ...
        for i in range(len(self.data)):
            ...
            # Argument
            arg = 1j * (interval['wavenum'] * xloc - interval['delta'])
            # The wavefunction
            yloc = interval['up'] * np.exp(arg) + interval['un'] * np.exp(-arg)
        ...
        # y = y / np.max(ymax) # reducing
        y_area = np.trapz(np.abs(y) ** 2, x) * 1e6
        # print np.max(y), np.sqrt(y_area)
        y = y / np.sqrt(y_area)  # Normalizing
        # print np.max(y)
        # print np.trapz(np.abs(y) ** 2, x)
        return x, y

    def plot_states(
            self, nwf=None, xmin=None, xmax=None, ymin=None, ymax=None):
        '''Plot wave functions (nwf stands for number of states to plot)'''
        toNm = 1e7
        self.eigen_energy = self.get_eigen_energy()
        if nwf is None or nwf > len(self.eigen_energy):
        ...
        pl.fill_between(bandx * toNm, bandy, y2=-2 * np.abs(ymin),
                        facecolor='0.95', color='0.95')
        ...
        for i in range(nwf):
            ...
            pl.plot(x, y, colors[i], lw=2, alpha=0.65,
                    label="%.3f meV" % (energy_ev * 1e3))
            pl.plot(xlvl, ylvl, colors[i], lw=1, alpha=0.5)
            pl.fill_between(x, y, y2=energy_ev, color=colors[i],
                            facecolor=colors[i], alpha='0.05', hatch=hatch[i])
        # Plot X limits
        fullwidth = np.sum([interval.width for interval in self.data[1:]])
        activereg = [interval.width for interval in self.data[1:]
                     if interval.label in ('well', 'stair')]
        ...
        if xmax is not None:
            ...
            pl.xlim(
                xmax=(fullwidth - self.data[-1].width + 5 * averageqw) * toNm)
    ...
    def reduce_wavefunction(self, y, value, dst=5):
        '''reduce_wavefunction(y, value, dst=5)
        Reducing wave function amplitude in order to fit a plot'''
        extrm = []
        for i in range(dst, len(y) - dst):
            y1 = np.abs(y[i - dst])
            y2 = np.abs(y[i + dst])
            if np.abs(y[i]) > max(y1, y2) and \
                    (y[i - dst + 1] - y1) * (y2 - y[i + dst - 1]) < 0:
                extrm.append(y[i])
        if extrm == []:
    ...
    def check_SQW(self):
        '''Check for a single quantum symmetric well with'''
        data = self.data[1:]
        er = self.get_energy_range()
        v = data[0].height
        m = data[1].mass * ME
        a = data[1].width

        func = lambda e: np.tan(
            0.5 * a * np.sqrt(2 * m * e) / HBAR) - np.sqrt((v - e) / e)

        print "Energy range:", erg2eV(er[1]), erg2eV(er[-1])
        pl.plot([erg2eV(e) for e in er], [func(e) for e in er])
        pl.plot([erg2eV(e) for e in er], [0 for e in er])
        pl.show()
        return erg2eV(self.bisection(er[1], er[-1], func))

    def bisection(self, x1, x2, func):
        '''Bisection method for quick minimum search'''
        x = (x1 + x2) / 2
        f = func(x)
        while (np.abs(f) > CALC_TOLERANCE):
            if (np.sign(f) * np.sign(func(x1)) > 0):
                x1 = x
            elif (np.sign(f) * np.sign(func(x2)) > 0):
                x2 = x
            if (x == (x1 + x2) * 0.5):
                return x
            x = (x1 + x2) / 2
...
if __name__ == '__main__':
    ...
    # i1 = MatterExpInterval(2e-5, 0.4, 0.067)
    # # i2 = MatterExpInterval(3e-7, 0,    0.067) # Well
    # i2 = MatterExpInterval(8e-7, 0, 0.067)  # Well
    # # i2 = MatterExpInterval(3e-6, 0,    0.067) # Well
    # i3 = MatterExpInterval(2e-5, 0.4, 0.067)
    ...
    num = 15
    widths     = np.random.random( num ) * 1e-6
    masses     = np.random.random( num ) / 4
    potentials = np.random.random( num ) / 3
    potentials = potentials - np.min( potentials )
    layers = []
    for i in xrange(15):
        layers = np.append( layers, MatterExpInterval( widths[i], potentials[i], 0.1 ) )
        # layers = np.append( layers, MatterExpInterval( widths[i], potentials[i], masses[i] ) )

    ms = MatterStack( layers )
    ms.get_eigen_energy()
    print "\n".join([str(erg2eV(e)) + " eV" for e in ms.get_eigen_energy()])
    ms.plot_states(nwf = 5)
    # ms.plot_states()

    # print "Check for a symmetric single quantum well"
    # e = ms.check_SQW()
